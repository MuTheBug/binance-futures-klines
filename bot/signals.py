"""
Live signal computation for the ECHO Engine.
...
echo.py) used in the backtest — so the bot trades exactly what was tested. Returns the
current target weights (fraction of equity per symbol, sign = long/short).
...
def _import_research(research_dir):
    ...
    import strategies, engine, echo  # noqa: E402
    return strategies, engine, echo
...
def compute_targets(client, cfg, log=print):
    ...
    """
    st, engine, echo = _import_research(cfg.research_dir)
