...
> **This branch's variant — combined book (ECHO v2 + RIFT).** `signals.py`
> blends the ECHO v2 book with the RIFT weekly cross-sectional momentum
> sleeve (`research/rift.py`), each vol-targeted separately, mixed by
> `BLEND_ECHO` (default 0.80/0.20). Backtest of the exact construction:
> `python3 -m seismo.combined` from the repo root. Defaults changed vs the
> original bot: `MAX_POSITIONS=30` (combined book holds ~27 names).
